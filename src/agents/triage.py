...
Returns a TriageResult that tells the runtime which workflow to invoke.
"""
import re
import sys
import os
...
# None hint means "let LookupAgent figure out the page from the index"
        ...
        response, usage = self._call(
            ...
            messages=[{
                ...
                "content": (
                    f"Command: {command}\n"
                    f"Error: {error}\n\n"
                    f"Existing wiki pages (for reference):\n{index_snippet}"
                )
            }],
            max_tokens=100,
        )
...
def _index_snippet() -> str:
    """Return just the page paths from index.md for the triage prompt."""
    import re as _re
    index = read_index()
    paths = _re.findall(r"\[\[([\w/.-]+)\]\]", index)
    return "\n".join(paths)
